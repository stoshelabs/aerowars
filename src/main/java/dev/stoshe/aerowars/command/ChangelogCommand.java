...
/** {@code /aerowars changelog} — re-opens the latest release notes popup (admin). */
public class ChangelogCommand extends AbstractPlayerCommand {
    private final AeroWars plugin;

    public ChangelogCommand(@Nonnull AeroWars plugin) {
        super("changelog", "Ver as novidades da última versão (admin)");
        this.plugin = plugin;
    }
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store,
        ...
        if (!plugin.getChangelogManager().isReady()) {
